	...
	with(ss){
		...
		if(v && document.getElementById(s[v-1]).selectedIndex>0 || !v){
			if(dsy.Exists(str)){
				...
				if($(ss).hasClass('iDoformHidden')){
					var tmpselect = $(ss);
					$(tmpselect).removeClass('iDoformHidden');
					var tmpselectparent = $(ss).parent().parent();
					$(ss).parent().remove();
					$(tmpselectparent).append(tmpselect);
					$(ss).jqTransSelect();
				}
			}
		}//end if v
...
var s=["iDo-province","iDo-city","iDo-county"];//三个select的name
var opt0 = ["请选择省份","请选择城市","请选择地区"];//初始值
