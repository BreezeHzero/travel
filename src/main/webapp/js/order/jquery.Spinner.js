(function($) {
	$.fn.Spinner = function (opts) {
		...
		var defaults = {value:1, min:1, len:3, max:99, readonly:"true"}
		...
		return this.each(function() {
			...
			p = $(this).parent().parent().parent().find('.iDoformCheckbox')
			p.hasClass('iDoformChecked')||p.click();
			});
			...
			localStorage.setItem("c",b.val());
		});

		function f(o,t,c,s){
			t==0&&o.addClass(c).attr("href","javascript:void(0)").append("<i></i>").find("i").append(s);
			t==1&&o.addClass(c).attr({"value":options.value,"autocomplete":"off","maxlength":options.len, "name":options.name,"readonly":options.readonly});
